...
void async function () {
  ...
  const t = []
  for (let i = 0; i < n; i++) {
    const [start, time] = (await readline()).split(' ')
    t.push({
  ...
  t.map(v => {
    if (v.time < 128) {
      min_t = Math.min(v.time + v.start, min_t)
    } else {
      let s = v.time.toString(2)
      let mant = parseInt(s.slice(1, 4), 2)
      let exp = parseInt(s.slice(5, 7), 2)
      min_t = Math.min(v.start + (mant | 0x10) << (exp + 3), min_t)
    }
  })
